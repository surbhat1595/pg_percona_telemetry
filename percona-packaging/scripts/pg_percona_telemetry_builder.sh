...
get_sources(){
    ...
    EDITFILES="debian/control debian/control.in debian/rules rpm/pg-percona-telemetry.spec"
    for file in $EDITFILES; do
        sed -i "s:@@PG_REL@@:${PG_RELEASE}:g" "$file"
    done

    sed -i "s:@@RPM_RELEASE@@:${RPM_RELEASE}:g" rpm/pg-percona-telemetry.spec
    sed -i "s:@@VERSION@@:${VERSION}:g" rpm/pg-percona-telemetry.spec

    set_changelog rpm/pg-percona-telemetry.spec
...
build_deb(){
    ...
    sed -i "s:\. :${WORKDIR}/percona-pg-telemetry-${VERSION} :g" debian/rules
